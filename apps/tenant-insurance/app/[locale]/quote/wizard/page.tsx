...
function WizardContent() {
  ...
  function onSubmit() {
    import("@/components/ui/sonner").then(({ toast }) =>
      toast.success(t('Wizard.submitOk.title'), { description: t('Wizard.submitOk.desc') })
    )
  }

  const planLabel = (p: 'Essential'|'Standard'|'Plus') => t(`Home.plan.${p}`)
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6">
          <div className="mb-4 text-sm text-muted-foreground">{t('Common.labels.step', {step: step + 1, total: steps.length, label: steps[step]})}</div>
                ...
                <RadioGroup value={form.watch("plan")} onValueChange={(v) => form.setValue("plan", v as FormValues["plan"]) } className="grid grid-cols-3 gap-2">
                  {(["Essential", "Standard", "Plus"] as const).map((p) => (
                    <div key={p} className="flex items-center gap-2 rounded-md border p-3">
                      ...
                      <Label htmlFor={`plan-${p}`} className="cursor-pointer">{planLabel(p)}</Label>
                    </div>
                  ))}
                </RadioGroup>
              </div>
          ...
          {step === 2 && (
            <div className="grid gap-6">
              <div className="grid gap-2">
                <Label>{t('Common.labels.personalProperty')} ({form.watch("propertyValue")})</Label>
              ...
              <div className="grid gap-2">
                <Label htmlFor="priorClaims">{t('Common.labels.priorClaims')}</Label>
          ...
          {step === 3 && (
            <div className="grid gap-6">
              <div className="text-sm text-muted-foreground">{t('Wizard.review.helper')}</div>
              <div className="grid gap-2">
                <div><span className="font-medium">{t('Wizard.review.plan')}</span> {planLabel(form.watch("plan"))}</div>
                <div><span className="font-medium">{t('Wizard.review.zip')}</span> {form.watch("zip")} <span className="font-medium">{t('Wizard.review.state')}</span> {form.watch("state")}</div>
                <div><span className="font-medium">{t('Wizard.review.property')}</span> ${form.watch("propertyValue")} <span className="font-medium">{t('Wizard.review.deductible')}</span> ${form.watch("deductible")}</div>
              </div>
              {quote && (
                <div className="grid gap-3">
                  <div className="text-lg font-semibold">{t('Wizard.review.estimated', {price: quote.premium})}</div>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{t('Common.labels.component')}</TableHead>
                        <TableHead className="text-right">{t('Common.labels.amount')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
          ...
          <div className="mt-6 flex items-center justify-between">
            <Button type="button" variant="secondary" onClick={prev} disabled={step === 0}>{t('Common.actions.back')}</Button>
            {step < steps.length - 1 ? (
              <Button type="button" onClick={next}>{t('Common.actions.next')}</Button>
            ) : (
              <Button type="button" onClick={onSubmit}>{t('Common.actions.submitApplication')}</Button>
            )}
          </div>
        </CardContent>
      </Card>

  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success(t('Claims.toastOk.title'), {
        description: data.id ? t('Claims.toastOk.desc', {id: data.id}) : undefined,
      })
    } catch {
      const { toast } = await import("@/components/ui/sonner")
            ...
            <CardContent>
              <form className="grid gap-4 sm:grid-cols-2" onSubmit={onSubmit}>
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="phone" inputMode="tel" placeholder={t('Common.placeholders.phone')} />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="date">{t('Common.labels.dateOfIncident')}</Label>
                  <Input id="date" type="date" required />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="type">{t('Common.labels.typeOfLoss')}</Label>
                  <Input id="type" placeholder={t('Common.placeholders.typeOfLoss')} required />
                </div>
                <div className="grid gap-2 sm:col-span-2">
                  <Label htmlFor="desc">{t('Common.labels.whatHappened')}</Label>
                  <Textarea id="desc" required placeholder={t('Common.placeholders.desc')} />
                </div>
                <div className="sm:col-span-2">
                  <Button type="submit">{t('Common.actions.submitClaim')}</Button>
                </div>
              </form>
            </CardContent>
          </>
      ...
      <p className="text-xs text-muted-foreground">
        {t.rich('Claims.trackLink', {
          here: (chunks: React.ReactNode) => (
            <Link href="/claims/track" className="underline">{chunks}</Link>
          )
        })}
      </p>
    </div>

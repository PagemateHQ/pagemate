  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">{t('Plans.title')}</h1>
        <p className="text-muted-foreground">{t('Plans.desc')}</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {plans.map((p) => (
          <Link href={{ pathname: "/quote", query: { plan: p.key } }} key={p.key}>
            <Card className="flex flex-col h-full">
              <CardHeader>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {p.popular ? <Badge variant="secondary">{t('Plans.popular')}</Badge> : null}
                </div>
              </CardHeader>
              <CardContent>
                <div>

...
export default async function PlanDetail({
	...
	const coverage =
		(t.raw(
			`Home.plan.features.${key === "Essential" ? "essential" : key === "Standard" ? "standard" : "plus"}`,
		) as string[]) ?? [];
	...
	return (
		<div className="space-y-6">
			...
			<Tabs defaultValue="coverage" className="w-full">
				<TabsList>
					<TabsTrigger value="coverage">
						{t("PlanDetails.tabs.coverage")}
					</TabsTrigger>
					<TabsTrigger value="exclusions">
						{t("PlanDetails.tabs.exclusions")}
					</TabsTrigger>
					<TabsTrigger value="endorsements">
						{t("PlanDetails.tabs.endorsements")}
					</TabsTrigger>
			...
			<div>
				<Button asChild>
					<Link href={{ pathname: "/quote/wizard", query: { plan: key } }}>
